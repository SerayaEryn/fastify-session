...
const CookieSecretsStore = require('./cookieSecretsStore')
...
function preValidation (options) {
  ...
  return function handleSession (request, reply, done) {
    ...
    const secret = options.secret.store[0]
    if (!sessionId) {
      newSession(secret, request, cookieOpts, done)
    } else {
      const l = options.secret.store.length
      let decryptedSessionId
      for (let i = 0; i < l; ++i) {
        decryptedSessionId = cookieSignature.unsign(sessionId, options.secret.store[i])
...
function checkOptions (options) {
  ...
  if (!(options.secret instanceof CookieSecretsStore)) {
    try {
      options.secret = new CookieSecretsStore(options.secret)
    } catch (e) {
      return e
    }
  }
...
module.exports.CookieSecretsStore = CookieSecretsStore
